...
const JWT_REFRESH_EXPIRES_IN: string = process.env.JWT_REFRESH_EXPIRES_IN ?? '7d' // Refresh token: 7일
const EMAIL_TOKEN_EXPIRES_IN: string = process.env.EMAIL_TOKEN_EXPIRES_IN ?? '24h' // 이메일 인증: 24시간
 ...
 * 이메일 인증 토큰 생성
 */
export function generateEmailVerificationToken(payload: Omit<JWTPayload, 'type' | 'iat' | 'exp'>): string {
 ...
 * 비밀번호 재설정 토큰 생성
 */
export function generatePasswordResetToken(payload: Omit<JWTPayload, 'type' | 'iat' | 'exp'>): string {
 ...
 * 토큰 타입 검증
 */
export function validateTokenType(token: string, expectedType: JWTPayload['type']): boolean {
  try {
    const payload = verifyToken(token)
