...
function ReviewDetailPage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        if (!data.success) {throw new Error(data?.error?.message ?? '로드 실패')}
        setReview(data.data.review)
        setComments(Array.isArray(data.data.review.comments) ? data.data.review.comments : [])
      } catch (e) {
  ...
  if (error || !review) {
    return <div className="container mx-auto px-4 py-6 text-red-600">{error ?? '리뷰를 찾을 수 없습니다'}</div>
  }
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-3xl">
      ...
      <div className="text-sm text-muted-foreground mb-4">
        {review.user?.nickname ?? '작성자'} · {new Date(review.createdAt).toLocaleString()}
      </div>
      {review.book && (
      ...
      <div className="flex items-center gap-2 mb-4">
        <Button variant={userHasLiked ? 'default' : 'outline'} size="sm" onClick={toggleLike}>
          {userHasLiked ? '좋아요 취소' : '좋아요'} ({likeCount})
        </Button>
        <div className="flex gap-2">
          {tags.map((t) => (
            <span key={t} className="px-2 py-1 text-xs border rounded-full">#{t}</span>
          ))}
        </div>
        <div className="ml-auto flex gap-2">
          <Button
            ...
            onClick={async () => {
              if (navigator.share) {
                try {
                  await navigator.share({ title: review.title, url: shareUrl, text: `${review.title} - ReadZone` })
                } catch {
      ...
      <section className="mt-8">
        ...
        {comments.length > 0 ? (
          <div className="space-y-4">
            {comments
              .filter((c) => !c.parentId)
              .map((c: { id: string; parentId?: string; user?: { nickname: string }; createdAt: string; content: string }) => {
                const replies = comments.filter((r) => r.parentId === c.id)

                return (
                  <div key={c.id} className="p-3 border rounded">
                    <div className="text-sm font-medium">{c.user?.nickname ?? '사용자'}</div>
                    <div className="text-xs text-muted-foreground mb-1">{new Date(c.createdAt).toLocaleString()}</div>
                    <div className="text-sm whitespace-pre-wrap mb-2">{c.content}</div>
                    {isAuthenticated && (
                      <div className="mb-2">
                        <Button
                          variant="outline"
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const res = await fetch(`${API_BASE_URL}/api/reviews/${reviewId}/comments`, {
                                  ...
                                  headers: { 'Content-Type': 'application/json' },
                                  credentials: 'include',
                                  body: JSON.stringify({ content, parentId: c.id }),
                                })
                                const data = await res.json()
                      ...
                      <div className="pl-4 border-l space-y-2">
                        {replies.map((r) => (
                          <div key={r.id} className="">
                            <div className="text-sm font-medium">{r.user?.nickname ?? '사용자'}</div>
