...
import { $createParagraphNode, $getRoot, $insertNodes, COMMAND_PRIORITY_LOW, DROP_COMMAND, type EditorState, PASTE_COMMAND } from 'lexical'
...
function ImageUploadPlugin({ onUpload }: { onUpload?: (file: File) => Promise<string> }) {
  ...
  return (
    <div className="p-2 border-b">
      <button type="button" className="px-2 py-1 text-sm border rounded" onClick={onClick}>이미지 업로드</button>
      <input ref={inputRef} type="file" accept="image/*" className="hidden" onChange={onFile} />
    </div>
  )
}
function PasteDropImagePlugin({ onUpload }: { onUpload?: (file: File) => Promise<string> }) {
  const [editor] = useLexicalComposerContext()
        ...
        ;(async () => {
          for (const f of images) {
            try {
              ...
              console.error(e instanceof Error ? e.message : '이미지 업로드 실패')
            }
          }
        })()
...
export default function LexicalEditor({ initialJson, placeholder, onChange, onImageUpload }: Props) {
  ...
  const initialConfig = useMemo(
    () => ({
      ...
      editorState: (editor: unknown) => {
        const ed = editor as { setEditorState: (state: unknown) => void; parseEditorState: (json: unknown) => unknown; update: (callback: () => void) => void }
  ...
  const handleChange = (editorState: EditorState, editor: unknown) => {
    ...
    editorState.read(() => {
      try {
        html = $generateHtmlFromNodes(editor as unknown as Parameters<typeof $generateHtmlFromNodes>[0])
      } catch {
        html = ''
      }
    })
    ...
    <LexicalComposer initialConfig={initialConfig}>
      <div className="border rounded-md">
        <div className="px-3 py-2 border-b text-sm text-muted-foreground">
          글을 자유롭게 작성하세요. 굵게/기울임/제목/목록/링크/코드와 Markdown 단축키를 지원합니다.
        </div>
        ...
        <div className="min-h-[240px] max-h-[60vh] overflow-y-auto">
          <RichTextPlugin
            contentEditable={<ContentEditable className="px-4 py-3 outline-none min-h-[240px]" />}
            placeholder={<div className="px-4 py-3 text-muted-foreground">{placeholder ?? '내용을 입력해 주세요...'}</div>}
            ErrorBoundary={LexicalErrorBoundary as unknown as Parameters<typeof RichTextPlugin>[0]['ErrorBoundary']}
          />
          <HistoryPlugin />
          <ListPlugin />
          <LinkPlugin />
