...
export const BadgeItem: React.FC<BadgeItemProps> = ({
  ...
  const badgeContent = (
    ...
    >
      ...
      <div className="relative z-10 text-center">
        {/* 아이콘 */}
        <div className={`${sizeStyle.icon} mb-2`}>
          {badge.icon}
        </div>

        {/* 배지 이름 */}
        <h3 className={`${sizeStyle.title} mb-1 line-clamp-1`}>
          {badge.name}
        </h3>

        {/* 배지 설명 */}
        <p className={`${sizeStyle.description} text-current opacity-75 line-clamp-2 mb-2`}>
          {badge.description}
        </p>
        ...
        {badge.isEarned && badge.earnedAt ? (
          ...
          <div className="space-y-1">
            ...
            <div className={`w-full bg-current/20 rounded-full ${sizeStyle.progress}`}>
              <div
  ...
  if (badge.isEarned) {
    return (
      <div className={`transition-all duration-300 ${isClickable ? 'hover:scale-105 active:scale-95' : ''}`}>
        {badgeContent}
      </div>
    )
  }
